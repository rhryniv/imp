"""Instance 1 lineage: I_0=[2.8,pi], I_1=[0,0.3] (RH's own choice) --
close to instance1_alt4_coarse_grid_early_stop_scan.py's I_0=[2.9,pi]
(found n=4 admissible, 18.5s), with I_0 widened slightly to start at 2.8
and I_1 narrowed to [0,0.3] (from [0.1,0.5]/[0.05,0.25] elsewhere).

M=25 (n_grid_B=n_grid_C=25) and the stop_at_first_admissible early exit
(driver.find_retained_design_stage3), n=2..16 (the usual default range
-- RH did not ask for a truncated range this time).

eps_0=eps_1=1e-2, N_max=60 (mu0=log(4/eps0)/(2*N_max)~=0.05).

Run from the repository root: python3 examples/instance1_alt6_early_stop_scan.py
"""
import json
import time
...
def main():
    ...
    save_records_csv(records, "examples/instance1_alt6_scan.csv")
    save_records_latex(records, "examples/instance1_alt6_scan_table.tex")
    with open("examples/instance1_alt6_scan.json", "w") as f:
        json.dump([r.to_json_dict() for r in records], f, indent=2)

    plot_bound_vs_achieved(records, retained=retained, savepath="examples/instance1_alt6_bound_vs_achieved.png")
    ...
    if plot_source is not None:
        plot_design_from_record(plot_source, I0, I1, N_values=(1, 3, 5), mu0=mu0_from_N_max(EPS0, N_MAX),
                                 savepath="examples/instance1_alt6_design.png")
        print(f"design plot uses n={plot_source.n} "
              f"({'retained' if retained is not None else 'best available, none admissible'})")

    print("\nSaved: examples/instance1_alt6_scan.{csv,json}, instance1_alt6_scan_table.tex, "
          "instance1_alt6_bound_vs_achieved.png, instance1_alt6_design.png")
