"""Instance 1, I_0=[1.0,1.5], I_1=[0.1,0.5] rerun with M=25
...
instance1_alt3_early_stop_scan.py (same intervals, M=200) never
completed -- the sandbox dropped it outright twice during unrelated
waiting, no results were ever produced. This is the first real attempt
at this interval choice, now also benefiting from the M=25 speedup.

eps_0=eps_1=1e-2, N_max=60 (mu0=log(4/eps0)/(2*N_max)~=0.05).

Run from the repository root: python3 examples/instance1_alt3_M25_early_stop_scan.py
"""
import json
import time
...
def main():
    ...
    save_records_csv(records, "examples/instance1_alt3_M25_scan.csv")
    save_records_latex(records, "examples/instance1_alt3_M25_scan_table.tex")
    with open("examples/instance1_alt3_M25_scan.json", "w") as f:
        json.dump([r.to_json_dict() for r in records], f, indent=2)

    plot_bound_vs_achieved(records, retained=retained, savepath="examples/instance1_alt3_M25_bound_vs_achieved.png")
    ...
    if plot_source is not None:
        plot_design_from_record(plot_source, I0, I1, N_values=(1, 3, 5), mu0=mu0_from_N_max(EPS0, N_MAX),
                                 savepath="examples/instance1_alt3_M25_design.png")
        print(f"design plot uses n={plot_source.n} "
              f"({'retained' if retained is not None else 'best available, none admissible'})")

    print("\nSaved: examples/instance1_alt3_M25_scan.{csv,json}, instance1_alt3_M25_scan_table.tex, "
          "instance1_alt3_M25_bound_vs_achieved.png, instance1_alt3_M25_design.png")
