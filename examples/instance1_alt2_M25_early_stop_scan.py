"""Instance 1, I_0=[1.5,2.0], I_1=[0.1,0.5] rerun with M=25
(n_grid_B=n_grid_C=25, down from the library default 200 -- RH's own
request) and the stop_at_first_admissible early exit
(driver.find_retained_design_stage3).

Compare against instance1_alt2_early_stop_scan.py (same intervals,
M=200): that run found n=9 admissible, but right at the constraint (E)
boundary (max_kappa_B=1.0000000006, 343.5s, 8 degrees computed). Whether
M=25 finds an earlier/more comfortable admissible degree, the same one,
or a worse outcome is exactly what this rerun checks.

eps_0=eps_1=1e-2, N_max=60 (mu0=log(4/eps0)/(2*N_max)~=0.05).

Run from the repository root: python3 examples/instance1_alt2_M25_early_stop_scan.py
"""
import json
import time
...
def main():
    ...
    save_records_csv(records, "examples/instance1_alt2_M25_scan.csv")
    save_records_latex(records, "examples/instance1_alt2_M25_scan_table.tex")
    with open("examples/instance1_alt2_M25_scan.json", "w") as f:
        json.dump([r.to_json_dict() for r in records], f, indent=2)

    plot_bound_vs_achieved(records, retained=retained, savepath="examples/instance1_alt2_M25_bound_vs_achieved.png")
    ...
    if plot_source is not None:
        plot_design_from_record(plot_source, I0, I1, N_values=(1, 3, 5), mu0=mu0_from_N_max(EPS0, N_MAX),
                                 savepath="examples/instance1_alt2_M25_design.png")
        print(f"design plot uses n={plot_source.n} "
              f"({'retained' if retained is not None else 'best available, none admissible'})")

    print("\nSaved: examples/instance1_alt2_M25_scan.{csv,json}, instance1_alt2_M25_scan_table.tex, "
          "instance1_alt2_M25_bound_vs_achieved.png, instance1_alt2_M25_design.png")
