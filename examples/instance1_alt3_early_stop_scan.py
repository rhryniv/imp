...
I_0=[1.0,1.5], I_1=[0.1,0.5] -- I_0 narrowed and moved closer to the
pass band than instance1_alt2_early_stop_scan.py's I_0=[1.5,2.0], same
I_1=[0.1,0.5]. Run with the N_max-derived gate plus the
...
Run from the repository root: python3 examples/instance1_alt3_early_stop_scan.py
"""
import json
import time
...
def main():
    ...
    save_records_csv(records, "examples/instance1_alt3_early_stop_scan.csv")
    save_records_latex(records, "examples/instance1_alt3_early_stop_scan_table.tex")
    with open("examples/instance1_alt3_early_stop_scan.json", "w") as f:
        json.dump([r.to_json_dict() for r in records], f, indent=2)

    plot_bound_vs_achieved(records, retained=retained,
                            savepath="examples/instance1_alt3_early_stop_bound_vs_achieved.png")
    ...
    if plot_source is not None:
        plot_design_from_record(plot_source, I0, I1, N_values=(1, 3, 5), mu0=mu0_from_N_max(EPS0, N_MAX),
                                 savepath="examples/instance1_alt3_early_stop_design.png")
        print(f"design plot uses n={plot_source.n} "
              f"({'retained' if retained is not None else 'best available, none admissible'})")

    print("\nSaved: examples/instance1_alt3_early_stop_scan.{csv,json}, instance1_alt3_early_stop_scan_table.tex, "
          "instance1_alt3_early_stop_bound_vs_achieved.png, instance1_alt3_early_stop_design.png")
